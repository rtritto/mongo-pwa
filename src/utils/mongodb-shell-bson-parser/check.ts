/** @link https://github.com/mongodb-js/devtools-shared/blob/main/packages/shell-bson-parser/src/check.ts */
...
class Checker {
  ...
  checkSafeCall = (node: BaseCallExpression) => {
    ...
    if (node.callee.type === 'Identifier') {
      return (
        GLOBAL_FUNCTIONS.includes(node.callee.name) &&
        node.arguments.every((node) => this.checkSafeExpression(node))
      )
    }
  ...
  checkSafeExpression = (node: Node | null): boolean => {
    switch (node?.type) {
      ...
      case 'ArrayExpression': {
        return node.elements.every((node) => this.checkSafeExpression(node))
      }
