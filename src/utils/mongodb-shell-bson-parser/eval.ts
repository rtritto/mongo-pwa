/** @link https://github.com/mongodb-js/devtools-shared/blob/main/packages/shell-bson-parser/src/eval.ts */

import type { UnaryExpression, BinaryExpression, Node, CallExpression, FunctionExpression, ArrowFunctionExpression } from 'estree'
...
const walk = (node: Node | null): any => {
  switch (node?.type) {
    ...
    default: {
      throw new Error('Invalid AST Node Provided')
    }
  }
}

export const executeAST = (node: Node) => {
  if (node.type === 'Program' && node.body.length === 1 && node.body[0].type === 'ExpressionStatement') {
