...
export default function parseRelaxedJSON(input: string): any {
  ...
  function object(): Record<string, any> {
    ...
    if (input.charCodeAt(pos) === 125) { pos++; return obj } // empty {}
    ...
    while (pos < len) {
      ...
      if (input.charCodeAt(pos) !== 58) fail('Expected ":" after property name') // :
      pos++
      ...
      if (c === 44) { // , (comma)
        pos++
        ws()
        if (input.charCodeAt(pos) === 125) { pos++; return obj } // Trailing comma handling
        continue
      }
  ...
  function array(): any[] {
    ...
    if (input.charCodeAt(pos) === 93) { pos++; return arr } // empty []
    ...
    while (pos < len) {
      ...
      if (c === 44) { // , (comma)
        pos++
        ws()
        if (input.charCodeAt(pos) === 93) { pos++; return arr } // Trailing comma handling
        continue
      }
  ...
  function string(): string {
    const quote = input.charCodeAt(pos++) // 34 (") or 39 (')
    let result = ''
    let start = pos
  ...
  function number(): number {
    ...
    // integer
    if (pos >= len || input.charCodeAt(pos) < 48 || input.charCodeAt(pos) > 57) fail('Invalid number')
    while (pos < len && input.charCodeAt(pos) >= 48 && input.charCodeAt(pos) <= 57) pos++
    ...
    // exponent
    if (pos < len && (input.charCodeAt(pos) === 101 || input.charCodeAt(pos) === 69)) { // e or E
      pos++
      if (pos < len && (input.charCodeAt(pos) === 43 || input.charCodeAt(pos) === 45)) pos++ // + or -
      while (pos < len && input.charCodeAt(pos) >= 48 && input.charCodeAt(pos) <= 57) pos++
    }

    const num = Number(input.slice(start, pos))
    if (!isFinite(num)) fail('Invalid number')
    return num
  }
